...
def split_dataset(
    ...
    formulas_files = sorted(
        [f for f in os.listdir(src_formulas_dir) if not f.endswith(".png")]
    )
...
def rename_files(validation_dir):
    ...
    for idx, (formula_file, image_file) in enumerate(
        zip(formulas_files, images_files), start=0
    ):
        new_formula_name = f"{idx}"
        new_image_name = f"{idx}.png"

        os.rename(
            os.path.join(formulas_dir, formula_file),
            os.path.join(formulas_dir, new_formula_name),
        )
        os.rename(
